    ...
    public class MyFileItPEMainService : IMyFileItPEMainService
    {
        ...
        public MyFileItResult AssociateDocumentsToTeamEventDocuments(string user, string pass, List<AssociateDocumentDTO> associations)
        {
            ...
            if (AllowAccess(user, pass))
            {
                ...
                {
                    var resp = AssociateDocumentToTeamEvent(user, pass, a.appUserId, a.organizationId, a.fileCabinetDocumentId, a.teamEventId, a.comment, a.emergency, a.remove);
                    if (!resp.Success)
                    {
                        success = false;
                    }
        ...
        public MyFileItResult ResendAssociatedDocuments(string user, string pass, int appUserId, int? teamEventId)
        {
            ...
            if (AllowAccess(user, pass))
            {
                ...
                if (emails.Any())
                {
                    ...
                    {
                        db.SHAREDOCUMENTs
                            .Where(sd => sd.APPUSERID == appUserId && sd.TEAMEVENTID == teamEventId)
                            .ToList().ForEach(newShare =>
                            {
                                newShare.APPUSER = db.APPUSERs.Single(au => au.ID == newShare.APPUSERID);
                                newShare.TEAMEVENT = db.TEAMEVENTs.Single(te => te.ID == teamEventId);
        ...
        public MyFileItResult AssociateDocumentToTeamEvent(string user, string pass, int appUserId, int organizationId, int fileCabinetDocumentId, int teamEventId, string comment, bool emergency, bool remove)
        {
            ...
            if (AllowAccess(user, pass))
            {
                using (var db = new MyFileItEntities())
                {
                    var share = db.SHAREDOCUMENTs.SingleOrDefault(s => s.TEAMEVENTID == teamEventId && s.FILECABINETDOCUMENTID == fileCabinetDocumentId);
                    ...
                    if (!remove)
                    {
                        if (share == null)
                        {
                            ...
                            var newShare = new SHAREDOCUMENT(appUserId, fileCabinetDocumentId, teamEventId, comment, emergency, emergencyEmailAddress, ref error);
        ...
        * ********************************************************/
        public MyFileItResult GetAppUserTeamEventDocumentsByTeamEvent(string user, string pass, int appUserId, int teamEventId)
        {
            ...
            if (AllowAccess(user, pass))
            {
                using (var db = new MyFileItEntities())
                {
                    db.SHAREDOCUMENTs.Where(sd => sd.APPUSERID == appUserId && sd.TEAMEVENTID == teamEventId)
                        .ToList()
                        .ForEach(sd =>
                        {
                            var entity = db.FILECABINETDOCUMENTs.Single(f => f.ID == sd.FILECABINETDOCUMENTID);
                            result.Documents.Add(new FileCabinetDocumentDTO(entity));
                            result.Success = true;
                        });
        ...
        * ********************************************************/
        ...
        public FileCabinetDocumentDTO GetSingleDocument(string user, string pass, FileCabinetDocumentSingleDTO lookup)
        {
            ...
            using (var db = new MyFileItEntities())
            {
                ...
                result.TeamEventId = lookup.TEAMEVENTID;
                //todo: NECESSARY?
        ...
        //                    if (teamEventId != null)
        //                    {
        //                        fileitDoc.TeamEventId = shareDocumentTeamEventIds.First(sdt => sdt.FileCabinetDocumentId == fd.ID).TeamEventDocumentId;
        ...
        private MyFileItResult FindDocumentsByIdListNoImages(List<int?> userIds, int? teamEventId, List<int?> skipDocumentList)
        {
            ...
            using (var db = new MyFileItEntities())
            {
                ...
                {
                    db.SHAREDOCUMENTs
                    .Where(sd => userIds.Contains(sd.APPUSERID) && sd.TEAMEVENTID == teamEventId)
                    .ToList()
                    .ForEach(sfd =>
                    {
                        var fileitDoc = new FileCabinetDocumentDTO(sfd.FILECABINETDOCUMENT);
                        fileitDoc.Base64Image = null;
                        fileitDoc.Base64ImageThumb = null;
        ...
        {
            var result = new MyFileItResult();
            var fileDocuments = new List<FILECABINETDOCUMENT>();
            ...
            using (var db = new MyFileItEntities())
            {
                if (teamEventId != null)
                {
                    ...
                    .ForEach(sfd =>
                    {
                        if (!skipDocumentList.Contains(sfd.FILECABINETDOCUMENTID))
                        {
                            fileDocuments.Add(sfd.FILECABINETDOCUMENT);
                        }
                    });
                }
                else
